...
func GetEtcdStorageDataForNamespaceServedAt(namespace string, v string, isEmulation bool) map[schema.GroupVersionResource]StorageData {
	...
	etcdStorageData := map[schema.GroupVersionResource]StorageData{
		...
		gvr("storage.k8s.io", "v1", "csinodes"): {
			...
			StatusStub:        `{"status": {"storageHealth": [{"name": "test-driver", "status": "StorageUnreachable", "reason": "ConnectionLost", "message": "lost connection", "lastTransitionTime": "2025-01-01T00:00:00Z"}]}}`,
			MutatedStatusStub: `{"status": {"storageHealth": [{"name": "test-driver", "status": "StorageDegraded", "reason": "HighLatency", "message": "high latency", "lastTransitionTime": "2025-01-01T00:00:00Z"}]}}`,
		},
		// --
