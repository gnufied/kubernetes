...
func (h *hostpathCSIDriver) PrepareTest(f *framework.Framework) (*testsuites.PerTestConfig, func()) {
	...
	cleanupFunc := func() {
		...
		// we need to remove the cleanupHandle at last so as if AfterEach and AfterSuite races with
		// each other and AfterEach(or defer) run gets killed then it still cleanups.
...
func (m *mockCSIDriver) PrepareTest(f *framework.Framework) (*testsuites.PerTestConfig, func()) {
	...
	cleanupFunc := func() {
		...
		ginkgo.By(fmt.Sprintf("deleting the driver namespace: %s", ns2))
		tryFunc(deleteNamespaceFunc(f.ClientSet, ns2, framework.DefaultNamespaceDeletionTimeout))
		// cleanup function has already ran and hence we don't need to run it again. We do this as very last action because
		// in case defer(or AfterEach) races with AfterSuite and test routine gets killed then this block still runs in AfterSuite.
