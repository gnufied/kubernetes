...
type VolumeFilter struct {
	...
	// IsMigrated returns a boolean specifying if the plugin has been migrated to CSI
...
var EBSVolumeFilter = VolumeFilter{
	...
	IsMigrated: func(csiNode *storagev1beta1.CSINode) bool {
		if !isCSIMigrationOn(csiNode, csilibplugins.AWSEBSInTreePluginName) {
			return false
		}
		return hasVolumeLimits(csiNode, csilibplugins.AWSEBSDriverName)
	},
}
...
var GCEPDVolumeFilter = VolumeFilter{
	...
	IsMigrated: func(csiNode *storagev1beta1.CSINode) bool {
		if !isCSIMigrationOn(csiNode, csilibplugins.GCEPDInTreePluginName) {
			return false
		}
		return hasVolumeLimits(csiNode, csilibplugins.GCEPDDriverName)
	},
}
...
var CinderVolumeFilter = VolumeFilter{
	...
	IsMigrated: func(csiNode *storagev1beta1.CSINode) bool {
		if !isCSIMigrationOn(csiNode, csilibplugins.CinderInTreePluginName) {
			return false
		}
		return hasVolumeLimits(csiNode, csilibplugins.CinderDriverName)
	},
}
