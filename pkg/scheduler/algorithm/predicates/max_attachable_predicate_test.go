...
func TestVolumeCountConflicts(t *testing.T) {
	...
	pluginMgr := getVolumePluginMgr()
	defer utilfeaturetesting.SetFeatureGateDuringTest(t, utilfeature.DefaultFeatureGate, features.DynamicVolumeThreshold, true)()

	for _, test := range tests {
		os.Setenv(KubeMaxPDVols, strconv.Itoa(test.maxVols))
		testPredicates := []struct {
			predicateFunc algorithm.FitPredicate
			name          string
		}{
			{
				NewMaxPDVolumeCountPredicate(test.filterName, pvInfo(test.filterName), pvcInfo(test.filterName)),
				"MaxPDVolumeCountPredicate",
			},
			{
				NewMaxAttachableVolumeLimitPredicate(pvInfo(test.filterName), pvcInfo(test.filterName), pluginMgr),
				"MaxAttachableVolumeLimitPred",
			},
		}
...
func getVolumePluginMgr() *volume.VolumePluginMgr {
	volumePlugins := volumeplugins.ProbeVolumePlugins()
	volumePluginMgr := &volume.VolumePluginMgr{}
	volumeHost := &volumeplugins.SchedulerVolumeHost{}
	if err := volumePluginMgr.InitPlugins(volumePlugins, nil /* prober */, volumeHost); err != nil {
...
func getVolumeLimitKey(filterType string) v1.ResourceName {
	...
	case EBSVolumeFilterType:
		return v1.ResourceName("storage-limits-aws-ebs")
	case GCEPDVolumeFilterType:
		return v1.ResourceName("storage-limits-gce-pd")
	case AzureDiskVolumeFilterType:
		return v1.ResourceName("storage-limits-azured-disk")
	default:
		return ""
	}
}
